...
# ── TRAINED MODEL INFERENCE ENGINE (XGBoost, trained in Google Colab) ──
try:
    from oracle_inference import run_inference, render_model_status_banner, models_available
    TRAINED_MODELS_AVAILABLE = models_available()
except ImportError:
    TRAINED_MODELS_AVAILABLE = False
    def render_model_status_banner(): return False
    def run_inference(r, h): return {"available": False, "error": "oracle_inference.py not found", "horizons": {}}
...
def run_full_analysis() -> dict:
    ...
    # ── ML: use pre-trained XGBoost if available, else fall back to session training ──
    if TRAINED_MODELS_AVAILABLE:
        _inf = run_inference(cbn_rate, st.session_state.rate_history)
        if _inf.get("available"):
            _h24  = _inf["horizons"].get("24h", {})
            _h48  = _inf["horizons"].get("48h", {})
            _h2   = _inf["horizons"].get("2h",  {})
            _h10  = _inf["horizons"].get("10h", {})
            ml = {
                "cold_start":        False,
                "n_training_points": _inf.get("metadata", {}).get("training_rows", 0),
                "model_type":        "XGBoost (pre-trained)",
                "ensemble":          _h24.get("rate", cbn_rate),
                "ridge_pred":        _h2.get("rate",  cbn_rate),
                "rf_pred":           _h10.get("rate", cbn_rate),
                "gb_pred":           _h48.get("rate", cbn_rate),
                "pred_low":          _h24.get("pred_low",  cbn_rate * 0.995),
                "pred_high":         _h24.get("pred_high", cbn_rate * 1.005),
                "confidence":        int(_h24.get("confidence", 50)),
                "direction":         (
                    "BULLISH" if (_h24.get("pct_change", 0) or 0) > 0.15
                    else "BEARISH" if (_h24.get("pct_change", 0) or 0) < -0.15
                    else "NEUTRAL"
                ),
                "model_agreement": max(0.0, 100.0 - abs(
                    (_h24.get("prob_up", 33) or 33) - (_h24.get("prob_down", 33) or 33)
                )),
                "inference_full": _inf,
            }
        else:
